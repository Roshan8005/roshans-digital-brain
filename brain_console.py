...
def print_help():
    ...
    print(
        "  /veda <mantra>      - Run Roshan's VedaKernel logic calculation and system status"
    )
    print(
        "  /simulate <num>     - Run synaptic signal propagation starting from a Neuron (1-86000)"
    )
            ...
            if user_input.startswith("/"):
                ...
                if cmd == "/exit":
                    ...
                    if os.path.exists(signal_script):
                        subprocess.run(
                            f'python "{signal_script}" --cleanup', shell=True
                        )
                    else:
                    ...
                    if not arg.isdigit():
                        print(
                            "[Error] /simulate requires a numeric Neuron ID (e.g. /simulate 1)"
                        )
                        continue
                    neuron_id = int(arg)
                    ...
                    if os.path.exists(signal_script):
                        ...
                        subprocess.run(
                            f'python "{signal_script}" --start {neuron_id} --message "Console_Triggered_Signal"',
                            shell=True,
                        )
                    else:
                        print("[Error] Signal script transmit_signal.py not found.")
                    ...
                    if not arg:
                        print(
                            "[Error] /query requires a search prompt (e.g. /query radiology)"
                        )
                        continue
                    ...
                    if "activated_regions" in res and res["activated_regions"]:
                        print(
                            f"[Activated Lobes]: {', '.join(res['activated_regions'])}"
                        )
                    print(f"\n{res['response']}")
                    ...
                    if not arg:
                        print(
                            "[Error] /veda requires a mantra input (e.g. /veda Agnimile)"
                        )
                        continue
