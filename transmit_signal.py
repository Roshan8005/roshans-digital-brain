...
def find_neuron_path(neuron_id):
    if 1 <= neuron_id <= 16340:
        ...
        return os.path.join(base_dir, "1_Cerebrum", lobes[lobe_idx], f"Gyrus_{gyrus_idx + 1}", f"Column_{col_idx + 1}", f"Neuron_{neuron_id}"), "Cerebrum"
        ...
        return os.path.join(base_dir, "2_Cerebellum", f"Lobule_{lobule_idx + 1}", f"Zone_{zone_idx + 1}", f"Microzone_{microzone_idx + 1}", f"Neuron_{neuron_id}"), "Cerebellum"
        ...
        return os.path.join(base_dir, "3_Brainstem", parts[part_idx], f"Nucleus_{nucleus_idx + 1}", f"Neuron_{neuron_id}"), "Brainstem"
        ...
        part_idx = (i % len(parts))
        nucleus_idx = (i // len(parts)) % 5
        return os.path.join(base_dir, "4_Limbic_System", parts[part_idx], f"Subnucleus_{nucleus_idx + 1}", f"Neuron_{neuron_id}"), "Limbic System"
        ...
        part_idx = (i % len(parts))
        nucleus_idx = (i // len(parts)) % 5
        return os.path.join(base_dir, "5_Diencephalon", parts[part_idx], f"Nucleus_{nucleus_idx + 1}", f"Neuron_{neuron_id}"), "Diencephalon"
    return None, "Unknown"
...
def get_synapses(neuron_id):
    ...
    # Limbic System projects to Cerebrum
        ...
        cerebrum_neuron = random.randint(1, 16340)
        connections.append(cerebrum_neuron)
    # Diencephalon projects to Cerebrum
    elif 85743 <= neuron_id <= 86000:
        cerebrum_neuron = random.randint(1, 16340)
        connections.append(cerebrum_neuron)
...
def run_simulation(start_id, message, max_hops):
    ...
    visited = set()
    queue = [(start_id, 0, f"Start (Neuron_{start_id})")]  # (neuron_id, hop_count, path_string)
    ...
    while queue:
        ...
        try:
            with open(signal_file, "w", encoding="utf-8") as f:
                f.write(f"--- Action Potential (Signal) ---\n")
    ...
    for hop, n_id, reg, folder in steps:
        ...
        print(f"{indent}{arrow}[Neuron_{n_id} in {reg}] (Signal file written: {os.path.basename(folder)}\\signal.txt)")
        
    print(f"\nTotal neurons activated: {len(steps)}")
    print(f"Created signal.txt files in all {len(steps)} folders to log active communication.")

def main():
    parser = argparse.ArgumentParser(description="Simulate synaptic communication between brain folder neurons.")
    parser.add_argument("--start", type=int, default=1, help="Starting neuron ID (1 - 86000)")
    parser.add_argument("--message", type=str, default="Action Potential Triggered", help="Signal message")
    parser.add_argument("--hops", type=int, default=3, help="Number of hops/levels of transmission")
    parser.add_argument("--cleanup", action="store_true", help="Delete all signal.txt files")
